...
const OptGraphData = ({ isOpen, onClose, serviceId }) => {
...
const barChartData = weekly.flatMap(item => 
  item.dailyCounts
    ...
    .map(dailyItem => ({
      ...
      pinverCountSuccess: dailyItem.pinVerScsCountOpt,
      cr: dailyItem.pinGenReqCount > 0
        ? ((dailyItem.pinVerScsCountOpt / dailyItem.pinGenSuccCount) * 100).toFixed(2)
        : 'N/A',
    }))
);
...
{barChartData && barChartData.length > 0 ? (
  <div className="graph-cont">
    ...
    <div className="graph">
      ...
      ((item.pinVerScsCountOpt / item.pingenCountSuccess) * 100).toFixed(2),
      100
      )}%
                 </td>
              </tr>
