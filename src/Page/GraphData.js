...
const GraphData = ({ isOpen, onClose, serviceId }) => {
  ...
  const chartData = selectedData
    ? selectedData.hours.map((hour, index) => ({
      ...
      pinverOptCountSuccess: hour.pinverOptCountSuccess,
    }))
    : [];
  ...
  const barChartData = weekly.flatMap(item =>
    item.dailyCounts
      ...
      .map(dailyItem => ({
        ...
        pinverOptCountSuccess: dailyItem.pinVerScsCountOpt,
        cr: dailyItem.pinGenReqCount > 0
          ? ((dailyItem.pinverCountSuccess / dailyItem.pinGenSuccCount) * 100).toFixed(2)
          : 'N/A',
      }))
  );
            ...
            <div className="graph">
              ...
              <div className="data-stats">
                <ul>
                  ...
                  <li>
                    CR(Opt): {pgsCount > 0 ? ((pinverOpt / pgsCount) * 100).toFixed(2) : 'N/A'}%
                  </li>
                </ul>
              </div>
            </div>
